<template>
  <div class="p-[10px] flex flex-col gap-5">
    ...
    <label class="input input-bordered flex items-center gap-2">
      ...
      <input v-model="inputApp" type="text" class="grow" placeholder="请输入百度应用ID"/>
    </label>
    <label class="input input-bordered flex items-center gap-2">
      <span class="font-bold text-primary">应用密钥：</span>
      <input v-model="inputKey" type="password" class="grow" placeholder="请输入百度应用秘钥"/>
    </label>
...
const engineInfo = computed(() => {
  return engineList.value.find(({name}) => name === "baidu");
});
...
const handleSaveButtonClick = async () => {
  await engineStore.setAppKey({engine: "baidu", app: inputApp.value, key: inputKey.value});
...
const alertInfo = computed(() => {
  ...
  return {type: "", content: "请提供『百度翻译』的应用 ID 和应用密匙"};
});
...
const applyUrl = "https://fanyi-api.baidu.com/choose";
